...
function selecionarQuestoesArray(questoes, totalPerguntas) {
  const embaralhaArray = questoes.sort(() => Math.random() - 0.5);

  return embaralhaArray.slice(0, totalPerguntas);
};

function exibirPerguntas(perguntas, index) {
  console.log(`${index + 1}: ${perguntas.pergunta}`);
  const resposta = obterDados.question("Digite a resposta (ano): ");

  return resposta;
};

function validarResposta(respostaUsuario, perguntas) {
  return respostaUsuario === perguntas.resposta;
};

function exibirResposta(portuacaoFinal, nomeJogador) {
  let mensagem = '';

  if(portuacaoFinal <= 3) {
    mensagem = '\nOH NÃO! Tente mais uma vez.';
  } else if(portuacaoFinal >= 4 && portuacaoFinal <= 6) {
    mensagem = '\nBOM TRABALHO! Pratique um pouco mais.';
  } else if(portuacaoFinal >= 7 && portuacaoFinal <= 9) {
  ...
  console.log(`Pontuação final: ${portuacaoFinal} acertos. `);
...
function iniciarQuiz() {
  ...
  const nomeJogador = obterDados.question("Digite o seu nome: ");
  const totalPerguntas = 10;
  const perguntasSelecionadas = selecionarQuestoesArray(questoes, totalPerguntas);

  let portuacaoFinal = 0;

  
  perguntasSelecionadas.forEach((perguntas, index) => {
    const respostaUsuario = exibirPerguntas(perguntas, index);
    const respostaValidada = validarResposta(respostaUsuario, perguntas);
  ...
  exibirResposta(portuacaoFinal, nomeJogador);
};
